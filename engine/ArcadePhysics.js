...
/*
	...
	Если сталкивается, то он должен произвести событие столкновения других
	объектов.
*/
/*
	export default означает, что экспортировать из файла
	нужно единственный класс - ArcadePhysics
*/
export default class ArcadePhysics {
	...
	// objects - объекты типа Display Objects
	add (...objects) {
	...
	processing () {
		...
		*/
		...
		for (let i = 0; i < objects.length - 1; i++) {
			...
			const topsA = a.tops
			/*
				Будем рассматривать, попадает ли точка в фигуру именно
				после передвижения (важна скорость).
			*/
			...
			for (let j = i + 1; j < objects.length; j++) {
				...
				const topsB = b.tops
				/*
					Будем рассматривать, попадает ли точка в фигуру именно после
					передвижения (важна скорость).
				*/
				...
				/*
					...
					попадает ли одна из точек вершин одного тела внутрь другого
					тела после передвижения.
				*/
				...
				/*
					Здесь проверяется, находится ли точка вершины
					прямоугольника A после перемещения внутри прямоугольника B
				...
				*/
				if (crossing === false) {
					/*
						Здесь проверяется, находится ли точка вершины
						прямоугольника B после перемещения
					...
					*/
					for (const topB of topsB) {
						...
						/*
							Если точка вершины прямоугольника B после
							перемещения находится внутри прямоугольника A после
							его перемещения:
						*/
						if (crossing) {
							break
				...
				if (crossing) {
					/*
						Передаем в каждый объект событие,
						тот элемент, с которым произошло столкновение,
						затем сам этот объект (самого себя).
					*/
